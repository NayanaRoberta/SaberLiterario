...
Este documento detalha como executar os testes de funcionalidade da API de Livros. A suíte de testes inclui tanto testes automatizados, para uma validação rápida e contínua, quanto testes manuais, para verificação em tempo real via Postman.
...
Observação: Recomenda-se criar uma Coleção no Postman para agrupar todas as requisições de teste. Isso facilita a importação e a execução por outros usuários.
